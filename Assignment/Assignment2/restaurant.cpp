...
public:
	...
	int hgt() {return this->height;}
	void updateHgt() {
		this->height = max(lc->hgt(), rc->hgt()) + 1;
	}
...
inline HuffTree* buildHuff(vector<pair<char,int>>& result) {
	...
	while(forest.size() > 1) { // O(n)
		...
		while(!nodeStack.empty()) {
			...
			if(newNode->right()) nodeStack.push(newNode->right());
			if(newNode->left()) nodeStack.push(newNode->left());
		}
...
public:
	...
	void BSTtoArray(BSTNode* root, vector<pair<int,int>>& arr) {
		// Convert cây BST trở về thành mảng theo dạng inorder
		if(!root) return;
	...
	void PascalTriangle(vector<vector<unsigned long long>>& pascalT, int N) {
		// i = số hàng
		for(long long int i = 0; i < N + 1; ++i) {
			pascalT[i] = vector<unsigned long long>(i + 1, 1); // hàng i có i+1 phần tử, khởi trị cho toàn bộ = 1
			for(long long int j = 1; j < i; ++j) {
				pascalT[i][j] = (pascalT[i - 1][j] + pascalT[i - 1][j - 1]); // Giá trị thằng hiện tại = 2 thằng trên cộng lại
			}
		}
	}
	unsigned long long int DFS(vector<pair<int,int>> arr, const vector<vector<unsigned long long>>& pascalT) {
		...
		return DFS(left, pascalT) * DFS(right, pascalT) * pascalT[arr.size() - 1][left.size()] ; 
	}
	void KOKUSEN() {
		for(int i = 0; i < GojoRestaurant->capacity; ++i) {
			...
			vector<vector<unsigned long long>> pascalT(arr.size() + 1);
			PascalTriangle(pascalT, arr.size());
			unsigned long long int Y = DFS(arr, pascalT);
			Y = Y % MAXSIZE;
