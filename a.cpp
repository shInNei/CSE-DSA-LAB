...
public:
	LeafNode(const char& key, int freq) { // Constructor Leaf Char
		this->key = key; this->wgt = freq; this->height = 0; lc = nullptr; rc = nullptr;
	}
...
public:
	...
	vector<pair<char,int>> frequency_sort(string& name) {
		...
		for(int i = 0; i < result.size(); i++) { // NHỚ XÓA CÁI NÀY GIÙM
	...
	void KOKUSEN() {
		...
		for(int i = 0; i < GojoRestaurant->capacity; ++i) {
			...
			int Y = countValidPermutations(GojoRestaurant->HashTable[i]->root, arr);
			Y = Y % MAXSIZE;
