...
public:
	...
	public:
		...
		void BSTtoArray(BSTNode* root, vector<int>& arr) {
			// Convert cây BST trở về thành mảng theo dạng postorder
			if(!root) return;
		...
		bool isEqual(BSTNode* a, BSTNode* b) {
			...
			if(!a && !b) return true;
				...
				return a->RESULT == b->RESULT && isEqual(a->lc, b->lc) && isEqual(a->rc, b->rc);
			}
			else return false;
		}
		int countValidPermutations(BSTNode* root, vector<int> arr) {
			int count = 0;
			BST* newTree = new BST();
			do {
